...
func (cmd *SchemaCmd) Run(globals *Globals) error {
	commands := []commandSchema{
		...
		{Name: "open", Summary: "Open a Slack archive URL as message, thread, or context", Examples: []string{"slacky open https://workspace.slack.com/archives/C123/p1717440000000000 --mode thread"}, ResultShape: "Envelope{message|thread|results}"},
		...
		{Name: "cache status", Summary: "Show cache status", Examples: []string{"slacky cache status --json"}, ResultShape: "Envelope{cache}"},
		...
		{Name: "auth import", Summary: "Import an existing Slack user token", Examples: []string{"slacky auth import", "slacky auth import --token-env SLACKY_USER_TOKEN"}, ResultShape: "Envelope{auth}"},
		{Name: "auth status", Summary: "Show redacted auth status", Examples: []string{"slacky auth status --json"}, ResultShape: "Envelope{auth}"},
		{Name: "auth logout", Summary: "Remove local Slack auth so the user can re-authenticate", Examples: []string{"slacky auth logout", "slacky auth logout --dry-run"}, ResultShape: "Envelope{auth,dry_run}"},
		...
		{Name: "skills get", Summary: "Print bundled runtime guidance as Markdown unless --json is explicit", Examples: []string{"slacky skills get core", "slacky skills get setup", "slacky skills get --all"}, ResultShape: "Markdown or Envelope{skills}"},
	}
