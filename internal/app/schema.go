...
func (cmd *SchemaCmd) Run(globals *Globals) error {
	commands := []commandSchema{
		...
		{Name: "find", Summary: "Rank conversations for consensus and recommendations; --local ranks cached messages without calling Slack", Examples: []string{"slacky find \"topic recommendations\" --json --compact", "slacky find --local \"topic recommendations\" --json --compact"}, ResultShape: "Envelope{threads,search}"},
		...
		{Name: "cache prune", Summary: "Preview or remove old cached messages by age while keeping FTS/thread rows in sync", Examples: []string{"slacky cache prune --older-than 90 --dry-run", "slacky cache prune --profile work --older-than 90 --apply"}, ResultShape: "Envelope{cache,dry_run}"},
		...
		{Name: "auth import-session", Summary: "Import an advanced Slack browser session using an xoxc token and d cookie value", Examples: []string{"slacky auth import-session --wizard --name work-browser", "slacky auth import-session --headless", "slacky auth import-session --xoxc-env SLACKY_XOXC --xoxd-env SLACKY_XOXD", "slacky auth import-session --wizard --name work-browser --user-agent \"$SLACKY_BROWSER_USER_AGENT\""}, ResultShape: "Envelope{auth|results}"},
		{Name: "auth list", Summary: "List saved auth profiles with active marker, token state, credential source, and storage labels", Examples: []string{"slacky auth list --json"}, ResultShape: "Envelope{auth}"},
		...
		{Name: "auth status", Summary: "Show redacted active auth status including source, storage, scopes, cache DB, expiry, refresh state, and mixed auth fields", Examples: []string{"slacky auth status --json", "slacky auth status --active --json"}, ResultShape: "Envelope{auth}"},
		...
		{Name: "skills get", Summary: "Print bundled runtime guidance as Markdown unless --json is explicit", Examples: []string{"slacky skills get core", "slacky skills get setup", "slacky skills get auth"}, ResultShape: "Markdown or Envelope{skills}"},
	}
	schema := map[string]any{
		...
		"search_syntax": map[string]any{
			...
			"local_search":           "search --local and find --local read the SQLite cache only and do not call Slack or expand handles/channels through Slack APIs.",
		},
	}
